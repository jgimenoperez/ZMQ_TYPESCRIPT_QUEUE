...
const recibe = zmq.socket("pull"),
  envia = zmq.socket("push"),
  publica = zmq.socket("pub"),
...
recibe.on("message", function (msg: any) {
  if (total_suscriptores == suscriptos) {
...
server.bind("tcp://127.0.0.1:3003", function (err: any) {
  ...
  else console.log("Listening on 3003...");
});
...
server.on("message", function (request: any) {
  ...
  console.log(request.toString(), suscriptos);
  server.send("OK");
